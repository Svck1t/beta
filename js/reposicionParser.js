...
const ReposicionParser = (() => {
  ...
  async function generateReposicionFiles(records) {
    ...
    const { workbook: vielWb, count: vielCount } = buildBodegaWorkbook(records, 'viel', 'VIEL');
    await downloadWorkbook(vielWb, vielFilename);

    const aldunateFilename = `Stock Aldunate ${fecha}.xlsx`;
    const { workbook: aldunateWb, count: aldunateCount } = buildBodegaWorkbook(records, 'aldunate', 'ALDUNATE');
    await downloadWorkbook(aldunateWb, aldunateFilename);

    return { vielFilename, aldunateFilename, vielCount, aldunateCount };
  }
